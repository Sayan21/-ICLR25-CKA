...
def Eval_Student(student,teacher,test_dataset, temp, batch_size):
    ...
    for batch in eval_dataloader:
        ...
        with torch.cuda.amp.autocast():
            ...
            #     loss[i+2] += CSLoss(masked_select(student_out.hidden_states[i+1],student_mask).view(-1,dS), masked_select(teacher_out.hidden_states[i+1],teacher_mask).view(-1,dT))
...
config = BertConfig.from_pretrained(teacher_id,hidden_size=student_dim,num_hidden_layers=n_layers, \
                                    intermediate_size = student_int_size, num_attention_heads=12,output_hidden_states = True)
...
optimizer = AdamW(optimizer_grouped_parameters , lr=2e-4, betas = (0.9,0.999), eps = 1e-6)
f = open("../../Checkpoints/LOGIT-KD-NH-EMB-12-%d-C4.txt" % student_dim, "w+", buffering= 50)
f1 = open("../../Checkpoints/LOGIT-KD-Eval-NH-EMB-12-%d-C4.txt" % student_dim, "w+", buffering= 1)
...
for batch in train_dataloader:
    ...
    with torch.cuda.amp.autocast():
        ...
        if(nBatch%num_batch_per_epoch==0):
            ...
            f1.flush()
            torch.save(student.state_dict(),"../../Checkpoints/Bert-12-%d-NH-C4.pt" % student_dim)
