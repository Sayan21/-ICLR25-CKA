...
def Eval_Student(student,teacher,test_dataset, batch_size = 32):
    ...
    eval_dataloader = DataLoader(test_dataset, shuffle=True, batch_size=32)
    student.eval()
    teacher.eval()
    ...
    for batch in eval_dataloader:
        ...
        with torch.cuda.amp.autocast():
            ...
            for i in range(n_layers):
                loss[i+2] += CSLoss(masked_select(student_out.hidden_states[i+1],student_mask).view(-1,dS), \
                               masked_select(teacher_out.hidden_states[4*(i+1)],teacher_mask).view(-1,dT))
...
for i in range(n_layers):
    student.bert.encoder.layer[i].attention.self.query.weight.copy_(teacher.bert.encoder.layer[4*i].attention.self.query.weight[:student_dim,:student_dim].detach().clone())
    student.bert.encoder.layer[i].attention.self.query.bias.copy_(teacher.bert.encoder.layer[4*i].attention.self.query.bias[:student_dim].detach().clone())
    student.bert.encoder.layer[i].attention.self.key.weight.copy_(teacher.bert.encoder.layer[4*i].attention.self.key.weight[:student_dim,:student_dim].detach().clone())
    student.bert.encoder.layer[i].attention.self.key.bias.copy_(teacher.bert.encoder.layer[4*i].attention.self.key.bias[:student_dim].detach().clone())
    student.bert.encoder.layer[i].attention.self.value.weight.copy_(teacher.bert.encoder.layer[4*i].attention.self.value.weight[:student_dim,:student_dim].detach().clone())
    student.bert.encoder.layer[i].attention.self.value.bias.copy_(teacher.bert.encoder.layer[4*i].attention.self.value.bias[:student_dim].detach().clone())

    
    student.bert.encoder.layer[i].attention.output.dense.weight.copy_(teacher.bert.encoder.layer[4*i].attention.output.dense.weight[:student_dim,:student_dim].detach().clone())
    student.bert.encoder.layer[i].attention.output.dense.bias.copy_(teacher.bert.encoder.layer[4*i].attention.output.dense.bias[:student_dim].detach().clone())
    student.bert.encoder.layer[i].attention.output.LayerNorm.weight.copy_(teacher.bert.encoder.layer[4*i].attention.output.LayerNorm.weight[:student_dim].detach().clone())
    student.bert.encoder.layer[i].attention.output.LayerNorm.bias.copy_(teacher.bert.encoder.layer[4*i].attention.output.LayerNorm.bias[:student_dim].detach().clone())

    student.bert.encoder.layer[i].intermediate.dense.weight.copy_(teacher.bert.encoder.layer[4*i].intermediate.dense.weight[:student_int_size,:student_dim].detach().clone())
    student.bert.encoder.layer[i].intermediate.dense.bias.copy_(teacher.bert.encoder.layer[4*i].intermediate.dense.bias[:student_int_size].detach().clone())

    student.bert.encoder.layer[i].output.dense.weight.copy_(teacher.bert.encoder.layer[4*i].output.dense.weight[:student_dim,:student_int_size].detach().clone())
    student.bert.encoder.layer[i].output.dense.bias.copy_(teacher.bert.encoder.layer[4*i].output.dense.bias[:student_dim].detach().clone())
    student.bert.encoder.layer[i].output.LayerNorm.weight.copy_(teacher.bert.encoder.layer[4*i].output.LayerNorm.weight[:student_dim].detach().clone())
    student.bert.encoder.layer[i].output.LayerNorm.bias.copy_(teacher.bert.encoder.layer[4*i].output.LayerNorm.bias[:student_dim].detach().clone())
...
f = open("../Checkpoints/LOGIT-KD-CKA-3-768-All.txt", "w+", buffering= 50)
f1 = open("../Checkpoints/LOGIT-KD-Eval-CKA-3-768-All.txt", "w+", buffering= 1)
...
for epoch in range(num_epochs):
    ...
    for batch in train_dataloader:
        ...
        with torch.cuda.amp.autocast():
            ...
            if(nBatch%10000==0):
                loss = Eval_Student(student,teacher,test_dataset, batch_size)
                print(epoch, ['%.4f' % l for l in loss])
                f1.write(str(['%.4f' % l for l in loss]) +  '\n')
                f1.flush()
                torch.save(student.state_dict(),"../Checkpoints/Bert-3-768-CKA-All-Wiki.pt")
