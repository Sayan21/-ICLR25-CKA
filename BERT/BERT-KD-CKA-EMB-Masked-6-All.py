    ...
    for batch in eval_dataloader:
        ...
        with torch.cuda.amp.autocast():
            ...
            for i in range(n_layers):
                loss[i+2] += CSLoss(masked_select(student_out.hidden_states[i+1],student_mask).view(-1,dS), masked_select(teacher_out.hidden_states[2*i+2],teacher_mask).view(-1,dT))
...
config = BertConfig.from_pretrained(teacher_id,hidden_size=student_dim,num_hidden_layers=n_layers, \
                                    intermediate_size = student_int_size, num_attention_heads=8,output_hidden_states = True)
    ...
    for batch in train_dataloader:
        ...
        with torch.cuda.amp.autocast():
            ...
            if(nBatch%num_batch_per_epoch==0):
                ...
                f1.flush()
                torch.save(student.state_dict(),"../../Checkpoints/Bert-6-%d-CKA-C4.pt" % student_dim)
