...
config = BertConfig.from_pretrained(teacher_id,hidden_size=student_dim,num_hidden_layers=student_layer, output_hidden_states = True, \
                                    intermediate_size = student_int_size, num_attention_heads=student_layer, num_labels = 3)
# student = BertForSequenceClassification(config)
# print(student.config)
# student.load_state_dict(torch.load('../../Checkpoints/BERT/Bert-%d-%d-%s-C4.pt' %(student_layer,student_dim,ident)), strict = False)

student = BertForSequenceClassification.from_pretrained("/data/projects/punim0478/sayantand/Checkpoints/BERT/minilm/%d_384" % student_layer,num_labels = 3)

student.to(device)
student.eval()
teacher.to(device)
teacher.eval()
metric = load('glue',dataset_config,experiment_id = "MNLI-%s-%d-%d.txt" %(ident,student_layer,student_dim) )
...
f1 = open("../../Checkpoints/LOGIT-KD-Eval-%s-BERT-MNLI-%d-%d.txt" % (ident,student_layer,student_dim), "w+", buffering= 1)
...
for epoch in range(num_epochs):
    ...
    torch.save(student.state_dict(),"../../Checkpoints/BERT-%s-MNLI-%d-%d.pt" % (ident,student_layer,student_dim))
