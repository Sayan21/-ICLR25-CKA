...
def Eval_Student(student,Eh,teacher,test_dataset, temp = 1.0, batch_size = 32):
    ...
    for batch in eval_dataloader:
        ...
        with torch.cuda.amp.autocast():
            ...
            j=2
            for i in range(4):
                loss[j] += CSLoss(Eh(masked_select(student_out.hidden_states[2*i+1],student_mask).view(-1,dS)), masked_select(teacher_out.hidden_states[3*i+2],teacher_mask).view(-1,dT))
                j+=1
                
            for i in range(4):
                loss[j] += CSLoss(Eh(masked_select(student_out.hidden_states[2*i+2],student_mask).view(-1,dS)), masked_select(teacher_out.hidden_states[3*i+3],teacher_mask).view(-1,dT))
                j+=1
...
f = open("../../Checkpoints/LOGIT-KD-TB-EMB-8-%d-C4.txt" % student_dim, "w+", buffering= 50)
f1 = open("../../Checkpoints/LOGIT-KD-Eval-TB-EMB-8-%d-C4.txt" % student_dim, "w+", buffering= 1)
    ...
    for batch in train_dataloader:
        ...
        with torch.cuda.amp.autocast():
            ...
            if(nBatch%vaild_sample_interval==0):
                ...
                f1.flush()
                torch.save(student.state_dict(),"../../Checkpoints/Bert-8-%d-TB-C4.pt" % student_dim)
