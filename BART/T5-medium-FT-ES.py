...
def Create_Student(student_id, teacher):
    ...
    student = T5ForConditionalGeneration.from_pretrained(student_id,num_layers = 12, num_decoder_layers=12,ignore_mismatched_sizes=True, output_hidden_states = True,output_past = False, use_cache=False)
...
tgt_lang = 'cot'
teacher_id = "google/t5-large-lm-adapt"
...
f = open("../../Checkpoints/LOGIT-KD-CKA-T5-%s-med-%d.txt" % (tgt_lang,LR), "w+", buffering= 20)
f1 = open("../../Checkpoints/LOGIT-KD-Eval-CKA-T5-%s-med-%d.txt" % (tgt_lang,LR), "w+", buffering= 1)
...
for epoch in range(num_epochs):
    ...
    for batch in train_dataloader:
        ...
        if(nBatch%vaild_sample_interval==0):
            torch.save(teacher.state_dict(),"../../Checkpoints/T5-medium-%s-%d.pt" % (tgt_lang,LR))
