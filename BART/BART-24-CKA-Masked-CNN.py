...
def Eval_Student(student,teacher,test_dataset, temperature = 1.0, batch_size = 32):
    from evaluate import load
    rouge = load('rouge', experiment_id = "Bart-CNN-%d-%d.txt" % (student.config.encoder_layers,student.config.d_model), use_stemmer = True)
...
config = BartConfig.from_pretrained('facebook/bart-large',d_model = student_dim, vocab_size = 50265, \
...
student.load_state_dict(torch.load('/data/projects/punim0478/sayantand/Checkpoints/BART/Bart-12-640-C4-CKA.pt'))
...
f = open("../../Checkpoints/LOGIT-KD-CKA-Bart-CNN-%d-%d.txt" % (student_layer,student_dim), "w+", buffering= 50)
f1 = open("../../Checkpoints/LOGIT-KD-Eval-CKA-Bart-CNN-%d-%d.txt" % (student_layer,student_dim), "w+", buffering= 1)
