...
def Eval_Student(student,teacher,test_dataset, temperature = 1.0, batch_size = 32):
    from evaluate import load
    rouge = load('rouge', experiment_id = "Bart-CNN-CKA-%d-%d-%d.txt" % (student.config.encoder_layers,student.config.d_model,LR), use_stemmer = True)
    ...
    for batch in eval_dataloader:
        ...
        predictions = student.generate(input_ids = batch['input_ids'],attention_mask = batch['attention_mask'], num_beams=6,max_length = 127)
...
# student.load_state_dict(torch.load('/data/projects/punim0478/sayantand/Checkpoints/BART/Bart-12-640-CKA.pt'))
...
############################ Concatenate CNN, CNN, CC NEWS & NEWSROOM ##########################
# dataset = load_dataset('EdinburghNLP/CNN')
    ...
    for batch in train_dataloader:
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            ...
            with torch.enable_grad():
                ...
                dV = student_out.logits.size(-1)
                logit_mask = batch['label_attention_mask'].unsqueeze(-1).expand_as(teacher_out.logits).bool()
                SL = masked_select(student_out.logits,logit_mask).view(-1,dV).float()
                loss[0] = CELoss(SL,masked_select(batch['label_ids'],batch['label_attention_mask'].bool()))
                loss[1] = ((temperature)**2)*STLoss(F.log_softmax(SL/ temperature,dim=-1), F.softmax(masked_select(teacher_out.logits,logit_mask).view(-1,dV).float()/ temperature, dim=-1))
        ...
        if(nBatch%vaild_sample_interval==0):     
            torch.save(student.state_dict(),"../../Checkpoints/Bart-%d-%d-%d-CNN-CKA.pt" % (student_layer,student_dim,LR))
...
torch.save(student.state_dict(),"../../Checkpoints/Bart-%d-%d-%d-XSUM-CKA.pt" % (student_layer,student_dim,LR))
