...
def Eval(teacher,test_dataset, batch_size = 32):
    bleu = load("sacrebleu",experiment_id = "T5-base-%s.txt" % tgt_lang, trust_remote_code=True)
    ...
    for batch in eval_dataloader:
        ...
        nBatch+=1
        if(nBatch%(vaild_sample_interval//5)==0): break    
...
tgt_lang = 'es'
teacher_id = "google/t5-v1_1-small"
...
f = open("../../Checkpoints/LOGIT-KD-CKA-T5-%s-base-%d.txt" % (tgt_lang,LR), "w+", buffering= 20)
f1 = open("../../Checkpoints/LOGIT-KD-Eval-CKA-T5-%s-base-%d.txt" % (tgt_lang,LR), "w+", buffering= 1)
...
for epoch in range(num_epochs):
    ...
    if(nBatch%vaild_sample_interval==0):
        torch.save(teacher.state_dict(),"../../Checkpoints/T5-base-%s-%d.pt" % (tgt_lang,LR))
