...
def Eval_Student(student,teacher,test_dataset, batch_size = 32):
    from evaluate import load
    rouge = load('rouge', experiment_id = "Bart-CNN-MSE-%d-%d.txt" % (student.config.encoder_layers,student.config.d_model), use_stemmer = True)
...
f = open("../../Checkpoints/LOGIT-KD-MSE-Bart-CNN-%d-%d.txt" % (student_layer,student.config.d_model), "w+", buffering= 50)
f1 = open("../../Checkpoints/LOGIT-KD-Eval-MSE-Bart-CNN-%d-%d.txt" % (student_layer,student.config.d_model), "w+", buffering= 1)
...
for epoch in range(num_epochs):
    ...
    for batch in train_dataloader:
        ...
        if(nBatch%vaild_sample_interval==0):
            torch.save(student.state_dict(),"../../Checkpoints/Bart-%d-%d-CNN-MSE.pt" % (student_layer,student.config.d_model))
