...
def Create_Student(student_id,student_dim,student_layer,teacher):
    config = MBartConfig(vocab_size = teacher.config.vocab_size, d_model = student_dim, \
                    encoder_layers = 12, decoder_layers = 3, \
    ...
    for i in range(student.config.encoder_layers):
        student.model.encoder.layers[i].self_attn.k_proj.weight.copy_(teacher.model.encoder.layers[i].self_attn.k_proj.weight.clone())
        student.model.encoder.layers[i].self_attn.k_proj.bias.copy_(teacher.model.encoder.layers[i].self_attn.k_proj.bias.clone())
        student.model.encoder.layers[i].self_attn.v_proj.weight.copy_(teacher.model.encoder.layers[i].self_attn.v_proj.weight.clone())
        student.model.encoder.layers[i].self_attn.v_proj.bias.copy_(teacher.model.encoder.layers[i].self_attn.v_proj.bias.clone())
        student.model.encoder.layers[i].self_attn.q_proj.weight.copy_(teacher.model.encoder.layers[i].self_attn.q_proj.weight.clone())
        student.model.encoder.layers[i].self_attn.q_proj.bias.copy_(teacher.model.encoder.layers[i].self_attn.q_proj.bias.clone())
        student.model.encoder.layers[i].self_attn.out_proj.weight.copy_(teacher.model.encoder.layers[i].self_attn.out_proj.weight.clone())
        student.model.encoder.layers[i].self_attn.out_proj.bias.copy_(teacher.model.encoder.layers[i].self_attn.out_proj.bias.clone())
    
        student.model.encoder.layers[i].self_attn_layer_norm.weight.copy_(teacher.model.encoder.layers[i].self_attn_layer_norm.weight.clone())
        student.model.encoder.layers[i].self_attn_layer_norm.bias.copy_(teacher.model.encoder.layers[i].self_attn_layer_norm.bias.clone())
    
        student.model.encoder.layers[i].fc1.weight.copy_(teacher.model.encoder.layers[i].fc1.weight.clone())
        ...
        student.model.encoder.layers[i].final_layer_norm.weight.copy_(teacher.model.encoder.layers[i].final_layer_norm.weight.clone())
        student.model.encoder.layers[i].final_layer_norm.bias.copy_(teacher.model.encoder.layers[i].final_layer_norm.bias.clone())
    ...
    for i in range(student.config.decoder_layers):
        student.model.decoder.layers[i].self_attn.k_proj.weight.copy_(teacher.model.decoder.layers[4*i].self_attn.k_proj.weight.clone())
        student.model.decoder.layers[i].self_attn.k_proj.bias.copy_(teacher.model.decoder.layers[4*i].self_attn.k_proj.bias.clone())
        student.model.decoder.layers[i].self_attn.v_proj.weight.copy_(teacher.model.decoder.layers[4*i].self_attn.v_proj.weight.clone())
        student.model.decoder.layers[i].self_attn.v_proj.bias.copy_(teacher.model.decoder.layers[4*i].self_attn.v_proj.bias.clone())
        student.model.decoder.layers[i].self_attn.q_proj.weight.copy_(teacher.model.decoder.layers[4*i].self_attn.q_proj.weight.clone())
        student.model.decoder.layers[i].self_attn.q_proj.bias.copy_(teacher.model.decoder.layers[4*i].self_attn.q_proj.bias.clone())
        student.model.decoder.layers[i].self_attn.out_proj.weight.copy_(teacher.model.decoder.layers[4*i].self_attn.out_proj.weight.clone())
        student.model.decoder.layers[i].self_attn.out_proj.bias.copy_(teacher.model.decoder.layers[4*i].self_attn.out_proj.bias.clone())
    
        student.model.decoder.layers[i].self_attn_layer_norm.weight.copy_(teacher.model.decoder.layers[4*i].self_attn_layer_norm.weight.clone())
        student.model.decoder.layers[i].self_attn_layer_norm.bias.copy_(teacher.model.decoder.layers[4*i].self_attn_layer_norm.bias.clone())
    
        student.model.decoder.layers[i].encoder_attn.k_proj.weight.copy_(teacher.model.decoder.layers[4*i].encoder_attn.k_proj.weight.clone())
        student.model.decoder.layers[i].encoder_attn.k_proj.bias.copy_(teacher.model.decoder.layers[4*i].encoder_attn.k_proj.bias.clone())
        student.model.decoder.layers[i].encoder_attn.v_proj.weight.copy_(teacher.model.decoder.layers[4*i].encoder_attn.v_proj.weight.clone())
        student.model.decoder.layers[i].encoder_attn.v_proj.bias.copy_(teacher.model.decoder.layers[4*i].encoder_attn.v_proj.bias.clone())
        student.model.decoder.layers[i].encoder_attn.q_proj.weight.copy_(teacher.model.decoder.layers[4*i].encoder_attn.q_proj.weight.clone())
        student.model.decoder.layers[i].encoder_attn.q_proj.bias.copy_(teacher.model.decoder.layers[4*i].encoder_attn.q_proj.bias.clone())
        student.model.decoder.layers[i].encoder_attn.out_proj.weight.copy_(teacher.model.decoder.layers[4*i].encoder_attn.out_proj.weight.clone())
        student.model.decoder.layers[i].encoder_attn.out_proj.bias.copy_(teacher.model.decoder.layers[4*i].encoder_attn.out_proj.bias.clone())
    
        student.model.decoder.layers[i].encoder_attn_layer_norm.weight.copy_(teacher.model.decoder.layers[4*i].encoder_attn_layer_norm.weight.clone())
        student.model.decoder.layers[i].encoder_attn_layer_norm.bias.copy_(teacher.model.decoder.layers[4*i].encoder_attn_layer_norm.bias.clone())
    
        student.model.decoder.layers[i].fc1.weight.copy_(teacher.model.decoder.layers[4*i].fc1.weight.clone())
        ...
        student.model.decoder.layers[i].final_layer_norm.weight.copy_(teacher.model.decoder.layers[4*i].final_layer_norm.weight.clone())
        student.model.decoder.layers[i].final_layer_norm.bias.copy_(teacher.model.decoder.layers[4*i].final_layer_norm.bias.clone())
...
def Eval_Student(student,teacher,test_dataset, batch_size = 32):
    bleu = load("sacrebleu",experiment_id = "MBart-%s-%d-%d.txt" % (src_lang,student.config.encoder_layers,student.config.d_model), trust_remote_code=True)
...
for i in range(1):
    ...
    student = MBartForConditionalGeneration(config)
    student.load_state_dict(torch.load("/data/projects/punim0478/sayantand/Checkpoints/BART/August_2024/MBart-CKA-%d-%d-MC4.pt" % (student_layer,student_dim)))

    student = Create_Student('...',student_dim,student_layer,student)
    ...
    f = open("../../Checkpoints/LOGIT-KD-CKA-MBart-%s-%d-%d.txt" % (src_lang,student_layer,student_dim), "w+", buffering= 50)
    f1 = open("../../Checkpoints/LOGIT-KD-Eval-CKA-MBart-%s-%d-%d.txt" % (src_lang,student_layer,student_dim), "w+", buffering= 1)
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_dataloader:
            ...
            if(nBatch%vaild_sample_interval==0):
                torch.save(student.state_dict(),"../../Checkpoints/MBart-%s-%d-%d-CKA.pt" % (src_lang,student_layer,student_dim))
